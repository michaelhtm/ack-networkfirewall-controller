...
func customCompareLoggingConfigurations(
	...
	// Sort a copy of the inputs to avoid modifying the original spec.
	desiredCopy := copySortedLoggingConfiguration(a.ko.Spec.LoggingConfiguration)
	latestCopy := copySortedLoggingConfiguration(b.ko.Spec.LoggingConfiguration)

	if len(desiredCopy.LogDestinationConfigs) != len(latestCopy.LogDestinationConfigs) {
		delta.Add("Spec.LoggingConfiguration", desiredCopy, latestCopy)
		return
	}

	if !reflect.DeepEqual(desiredCopy.LogDestinationConfigs, latestCopy.LogDestinationConfigs) {
		delta.Add("Spec.LoggingConfiguration", desiredCopy, latestCopy)
	}
}

func copySortedLoggingConfiguration(
	loggingConfiguration *v1alpha1.LoggingConfiguration,
) *v1alpha1.LoggingConfiguration {
	if loggingConfiguration == nil {
		return &svcapitypes.LoggingConfiguration{
			LogDestinationConfigs: []*svcapitypes.LogDestinationConfig{},
		}
	}

	newLoggingConfig := &v1alpha1.LoggingConfiguration{}
	newLoggingConfig.LogDestinationConfigs = make([]*svcapitypes.LogDestinationConfig, len(loggingConfiguration.LogDestinationConfigs))
	for i, logDestinationConfigs := range loggingConfiguration.LogDestinationConfigs {
		newMap := make(map[string]*string, len(logDestinationConfigs.LogDestination))
		for key, val := range logDestinationConfigs.LogDestination {
			newMap[key] = val
		}
		newLoggingConfig.LogDestinationConfigs[i] = &v1alpha1.LogDestinationConfig{
			LogDestination:     newMap,
			LogDestinationType: logDestinationConfigs.LogDestinationType,
			LogType:            logDestinationConfigs.LogType,
		}
	}

	sort.Slice(newLoggingConfig.LogDestinationConfigs[:], func(i, j int) bool {
		return *newLoggingConfig.LogDestinationConfigs[i].LogType < *newLoggingConfig.LogDestinationConfigs[j].LogType
	})
...
func (rm *resourceManager) syncLoggingConfiguration(
	...
	var desiredLogDestinationConfig, latestLogDestinationConfig []*svcapitypes.LogDestinationConfig

	if desired != nil && desired.ko.Spec.LoggingConfiguration != nil {
		desiredLogDestinationConfig = desired.ko.Spec.LoggingConfiguration.LogDestinationConfigs
	}

	if latest != nil && latest.ko.Spec.LoggingConfiguration != nil {
		latestLogDestinationConfig = latest.ko.Spec.LoggingConfiguration.LogDestinationConfigs
	}

	if latest != nil && desired != nil {
		add, delete, err = compareLoggingDestinationConfigs(desiredLogDestinationConfig, latestLogDestinationConfig)
		if err != nil {
			return err
		}
